...
fn inspect(reference_to_string: &str) {
    println!("{}", reference_to_string.ends_with("s"))
}

fn change(reference_to_string: &mut String) {
    if !reference_to_string.ends_with("s") {
        reference_to_string.push('s');
    }
}

fn eat(moved_string: String) -> bool {
    moved_string.starts_with("b") && moved_string.contains("a")
}

fn bedazzle(mutable_reference_to_string: &mut String) {
    *mutable_reference_to_string = "sparkly".to_string();
}
