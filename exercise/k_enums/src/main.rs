...
fn main() {
    ...
    impl Snack {
        fn price(self) -> u8 {
            match self {
                ...
                Snack::Sandwich { lettuce, cheese } => 10 + (lettuce as u8) + (cheese as u8) * 2,
            }
        }
    }
    ...
    for (index, snack) in snacks.iter().enumerate() {
        match snack {
            Snack::Apple => println!("Snack {} is an apple.", index),
            Snack::Cookies(number) => println!("Snack {} is {} cookies.", index, number),
            Snack::Sandwich { lettuce, cheese } => {
                let lettuce_message = if *lettuce { " with lettuce" } else { "" };
                let cheese_message = if *cheese { " with cheese" } else { "" };
                println!(
                    "Snack {} is a sandwich{}{}.",
                    index, lettuce_message, cheese_message
                );
            }
        }
    }
